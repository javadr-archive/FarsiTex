...
SfGoods.prototype.toggleCheckboxInfo = function(){
	infoEl = jQuery(this).parent('label').parent('p').next('div');
	if (!infoEl.is('.topform')){
		if (jQuery(this).is(":checked")){
			infoEl.addClass('info').show();
	    } else {
	    	infoEl.hide();
	    }
    }
...
function visChecked() {
	if (jQuery(this).is(":checked")) {
		jQuery(this).parent('label').parent('p').next('div').addClass('helptip').removeClass('hide');
	} else {
		jQuery(this).parent('label').parent('p').next('div').removeClass('helptip').addClass('hide');
	}
}
...
jQuery(document).ready(function(){
    visChecked();
    jQuery(":checkbox").each(visChecked);
    jQuery(":checkbox").change(visChecked);

    var sfGoods = new SfGoods();
    sfGoods.initFormNoteBehavior();
    jQuery('#topuser a:gt(0)').addClass('menu1');
    jQuery("#kahuna form input#searchsubmit").mousedown(function(){jQuery(this).addClass('test');});
    jQuery("#kahuna form input#searchsubmit").mouseup(function(){jQuery(this).removeClass('test');});
    ...
    jQuery('label').click(function(){
	    radioInputEl = jQuery(this).find("input[@type='radio']");
		if (radioInputEl.is(':checked')){
